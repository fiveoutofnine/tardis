...
import {WhatAreBuckets} from "src/curta/eth/4/WhatAreBuckets.sol";
import {CurtaSolution} from "test/utils/CurtaSolution.sol";

abstract contract Setup is CurtaSolution(1, 4) {
    // -------------------------------------------------------------------------
    // Immutable storage
    // -------------------------------------------------------------------------
    ...
    function setUp() public virtual override {
        ...
        puzzle = IPuzzle(new WhatAreBuckets());
        vm.label(address(puzzle), string.concat("Puzzle #4: ", puzzle.name()));
