...
import {Lana} from "src/curta/eth/20/Lana.sol";

import {CurtaSolution} from "test/utils/CurtaSolution.sol";

abstract contract Setup is CurtaSolution(1, 20) {
    // -------------------------------------------------------------------------
    // Immutable storage
    // -------------------------------------------------------------------------
    ...
    function setUp() public virtual override {
        ...
        puzzle = IPuzzle(address(new Lana()));
        vm.label(address(puzzle), "Puzzle #20: Lana");

        // Add puzzle to Curta as `mockAuthor`.
        vm.prank(mockAuthor);
        curta.addPuzzle(puzzle, 20);
    }
}
