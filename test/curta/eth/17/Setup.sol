...
import {MurderMystery} from "src/curta/eth/17/MurderMystery.sol";

import {CurtaSolution} from "test/utils/CurtaSolution.sol";

abstract contract Setup is CurtaSolution(1, 17) {
    // -------------------------------------------------------------------------
    // Immutable storage
    // -------------------------------------------------------------------------
    ...
    function setUp() public virtual override {
        ...
        puzzle = IPuzzle(address(new MurderMystery()));
        vm.label(address(puzzle), "Puzzle #17: Murder Mystery");

        // Add puzzle to Curta as `mockAuthor`.
        vm.prank(mockAuthor);
        curta.addPuzzle(puzzle, 17);
    }
}
