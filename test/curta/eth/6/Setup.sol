...
import {EventHorizon} from "src/curta/eth/6/EventHorizon.sol";
import {CurtaSolution} from "test/utils/CurtaSolution.sol";

abstract contract Setup is CurtaSolution(1, 6) {
    // -------------------------------------------------------------------------
    // Immutable storage
    // -------------------------------------------------------------------------
    ...
    function setUp() public virtual override {
        ...
        puzzle = IPuzzle(new EventHorizon());
        vm.label(address(puzzle), string.concat("Puzzle #6: ", puzzle.name()));

        // Add puzzle to Curta as `mockAuthor`.
        vm.prank(mockAuthor);
        curta.addPuzzle(puzzle, 6);
    }
}
