...
import {Puzzle} from "src/curta/eth/16/Elohim.sol";

import {CurtaSolution} from "test/utils/CurtaSolution.sol";

abstract contract Setup is CurtaSolution(1, 16) {
    // -------------------------------------------------------------------------
    // Immutable storage
    // -------------------------------------------------------------------------
    ...
    function setUp() public virtual override {
        ...
        address nerv = HuffDeployer.deploy("curta/eth/16/NERVCommand");
        puzzle = IPuzzle(new Puzzle(nerv));
        vm.label(address(puzzle), "Puzzle #16: Elohim");

        // Add puzzle to Curta as `mockAuthor`.
        vm.prank(mockAuthor);
        curta.addPuzzle(puzzle, 16);
    }
}
