...
library EvmLib {
    ...
    function newEvm(EvmContext memory ctx) internal pure returns (Evm e) {
    ...
    function compress(
        ...
        uint256 op,
        uint256 ptr16bit
    ) internal pure returns (uint256 ps) {
    ...
    function evaluate(
        Evm self,
        bytes memory bytecode,
        uint16 stackSizeHint,
        uint16 storageSizeHint,
        uint32 memSizeHint
    ) internal returns (bool success, bytes memory ret) {
        ...
        for (uint256 i; i < bcodeLen; ++i) {
            ...
            if (op == 0) {
                ...
                if (op >= 0x56 && op <= 0x58) {
                    ...
                    (stack, mem, store, ctx) = intoOp(uncompress(ops, op))(
                        mem,
                        stack,
                        store,
                        ctx
    ...
    function push(
        ...
        uint256 op,
        uint256 i
    ) internal view returns (Stack s, uint256 j) {
        if (op == 0x5f) {
            s = self.push(0);
            return (s, i);
        }
    ...
    function intoPtr(
        function(Memory, Stack, Storage, bytes32)
            internal
            view
            returns (Stack, Memory, Storage, bytes32) op
    ) internal pure returns (uint256 ptr) {
        assembly ("memory-safe") {
            ptr := op
        }
    ...
    function intoPushPtr(
        function(Stack, bytes memory, uint256, uint256)
            internal
            view
            returns (Stack, uint256) op
    ) internal pure returns (uint256 ptr) {
        assembly ("memory-safe") {
            ptr := op
        }
    ...
    function intoDupOpPtr(
        function(Stack, uint256) view returns (Stack) op
    ) internal pure returns (uint256 ptr) {
        assembly ("memory-safe") {
            ptr := op
        }
    ...
    function intoDupOp(
        uint256 ptr
    ) internal pure returns (function(Stack, uint256) view returns (Stack) op) {
        assembly ("memory-safe") {
            op := ptr
        }
    }

    function intoSwapOpPtr(
        function(Stack, uint256) op
    ) internal pure returns (uint256 ptr) {
        assembly ("memory-safe") {
            ptr := op
        }
    ...
    function intoSwapOp(
        uint256 ptr
    ) internal pure returns (function(Stack, uint256) view op) {
        assembly ("memory-safe") {
            op := ptr
        }
    }
