...
library IndexableLinkedListLib {
    ...
    function newIndexableLinkedList(
        uint16 capacityHint
    ) internal pure returns (LinkedList s) {
    ...
    function get(
        ...
        elementPointer = bytes32(
            Array.wrap(LinkedList.unwrap(self)).get(index)
        );
    }
    ...
    function unsafe_get(
        ...
        elementPointer = bytes32(
            Array.wrap(LinkedList.unwrap(self)).unsafe_get(index)
        );
    }
}
