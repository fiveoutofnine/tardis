...
library IndexableDoublyLinkedListLib {
    ...
    function newIndexableDoublyLinkedList(
        uint16 capacityHint
    ) internal pure returns (DoublyLinkedList s) {
        s = DoublyLinkedList.wrap(
            Array.unwrap(ArrayLib.newArray(capacityHint))
        );
    }
    ...
    function push_no_link(
        DoublyLinkedList self,
        bytes32 element
    ) internal view returns (DoublyLinkedList s) {
        s = DoublyLinkedList.wrap(
            Array.unwrap(
    ...
    function push_and_link(
        DoublyLinkedList self,
        bytes32 element,
        uint256 backwardLinkingOffset,
        uint256 forwardLinkingOffset
    ) internal view returns (DoublyLinkedList s) {
    ...
    function get(
        DoublyLinkedList self,
        uint256 index
    ) internal pure returns (bytes32 elementPointer) {
        elementPointer = bytes32(
            Array.wrap(DoublyLinkedList.unwrap(self)).get(index)
        );
    }
    ...
    function unsafe_get(
        ...
        elementPointer = bytes32(
            Array.wrap(DoublyLinkedList.unwrap(self)).unsafe_get(index)
        );
    }
}
...
library DoublyLinkedListLib {
    uint256 constant HEAD_MASK =
        0xFFFFFFFFFFFFFFFFFFFF00000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF;
    uint256 constant TAIL_MASK =
        0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000000000000000000000;

    function newDoublyLinkedList(
        uint40 _backwardLinkingOffset,
        uint40 _forwardLinkingOffset
    ) internal pure returns (DoublyLinkedList s) {
        assembly ("memory-safe") {
            s := shl(
                176,
                or(shl(40, _backwardLinkingOffset), _forwardLinkingOffset)
            )
        }
    }
    ...
    function forwardLinkingOffset(
        DoublyLinkedList s
    ) internal pure returns (uint80 offset) {
        assembly ("memory-safe") {
            offset := shr(216, shl(40, s))
        }
    }

    function backwardLinkingOffset(
        DoublyLinkedList s
    ) internal pure returns (uint80 offset) {
        assembly ("memory-safe") {
            offset := shr(216, s)
    ...
    function push_and_link(
        ...
        assembly ("memory-safe") {
            ...
            case 1 {
                ...
                mstore(add(element, shr(216, self)), tailElement)
