...
library JsonLib {
    ...
    function uncheckedInsert(
        ...
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Json.unwrap(self)).unsafe_get(bucket))
        );
        Array.wrap(Json.unwrap(self)).unsafe_set(
            bucket,
            uint256(LinkedList.unwrap(linkedList.push_and_link(entryPtr)))
        );
    }
    ...
    ) internal pure returns (bool found, uint256 val) {
        uint256 bucket = uint256(key) % buckets(self);
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Json.unwrap(self)).unsafe_get(bucket))
        );
    ...
    function checkType(
        Json self,
        bytes32 key
    ) internal pure returns (bool found, JsonType t) {
        uint256 bucket = uint256(key) % buckets(self);
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Json.unwrap(self)).unsafe_get(bucket))
        );
    ...
    function getString(
        Json self,
        bytes32 key
    ) internal pure returns (bool found, string memory str) {
    ...
    function getObject(
        Json self,
        bytes32 key
    ) internal pure returns (bool found, Json obj) {
    ...
    function getArray(
        Json self,
        bytes32 key
    ) internal pure returns (bool found, Array arr) {
    ...
    function getBool(
        Json self,
        bytes32 key
    ) internal pure returns (bool found, bool val) {
