...
library MappingLib {
    ...
    function newMapping(uint16 capacityHint) internal pure returns (Mapping s) {
        ...
        for (uint256 i; i < capacityHint; i++) {
            ...
            uint256 linkedListPtr = uint256(
                LinkedList.unwrap(LinkedListLib.newLinkedList(0x40))
            );
    ...
    function uncheckedInsert(
        ...
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Mapping.unwrap(self)).unsafe_get(bucket))
        );
        Array.wrap(Mapping.unwrap(self)).unsafe_set(
            bucket,
    ...
    function update(
        ...
        bytes32 key,
        uint256 value
    ) internal pure returns (bool wasSet) {
        uint256 bucket = uint256(key) % buckets(self);
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Mapping.unwrap(self)).unsafe_get(bucket))
        );
    ...
    function containsKey(
        Mapping self,
        bytes32 key
    ) internal pure returns (bool hasKey) {
        uint256 bucket = uint256(key) % buckets(self);
        LinkedList linkedList = LinkedList.wrap(
            bytes32(Array.wrap(Mapping.unwrap(self)).unsafe_get(bucket))
        );
